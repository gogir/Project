...
(function($) {
	...
	function _9e(_9f) {
		...
		for (var _a4 = 0; _a4 < aa.length; _a4++) {
			...
			if (_a5 == "list") {
				...
				$("<td>每页</td>").append(ps).append("条").appendTo(tr); // diy 修改页码数据
			} else {
				if (_a5 == "sep") {
		...
		function _a6(_a8) {
			var btn = _a1.nav[_a8];
			//原代码 var a = $("<a href=\"javascript:void(0)\"></a>").appendTo(tr);
			//diy 增加分页提示信息
			var title = "";
			if (_a8 == "first") title = "第一页";
			else if (_a8 == "prev") title = "上一页";
			else if (_a8 == "next") title = "下一页";
			else if (_a8 == "last") title = "最后一页";
			else if (_a8 == "refresh") title = "刷新";

			var a = $("<a href=\"javascript:void(0)\" title=\"" + title + "\"></a>").appendTo(tr);
			//
			a.wrap("<td></td>");
			a.linkbutton({
				iconCls: btn.iconCls,
	...
	$.fn.textbox.defaults, {
		...
		filter: function(e) {
			...
			var numtext = $(this).numberbox("getText"); //diy bug 不能取val值
			if (e.which == 45) {
				return (numtext.indexOf("-") == -1 ? true: false);
			}
			var c = e.which==46 ? "." : String.fromCharCode(e.which);
			if (c == opts.decimalSeparator) {
				return (numtext.indexOf(c) == -1 ? true: false);
			} else {
	...
	function _5a4(_5a5) {
		...
		function _5aa(_5af, _5b0, _5b1) {
			...
			for (var i = 0; i < _5b0.length; i++) {
				...
				for (var j = 0; j < cols.length; j++) {
					...
					if (col.checkbox) {
						...
						//diy 2014-09-28  修改如果只允许单选，则行头不显示选择框
						//$("<div class=\"datagrid-header-check\"></div>").html("<input type=\"checkbox\"/>").appendTo(td);
						if(opts.singleSelect)
							$("<div class=\"datagrid-header-check\"></div>").html("").appendTo(td);
						else
							$("<div class=\"datagrid-header-check\"></div>").html("<input type=\"checkbox\"/>").appendTo(td);
					} else {
						if (col.field) {
			...
			if (_5b1 && opts.rownumbers) {
				var td = $("<td rowspan=\"" + opts.frozenColumns.length + "\"><div class=\"datagrid-header-rownumber\">行号</div></td>");
				if ($("tr", t).length == 0) {
	...
	function _632(_633, _634, _635) {
		...
		var trs = opts.finder.getTr(_633, "", "checked");

		if (opts.singleSelect || trs.length == 0) { //只允许单选或者不存在已经选中的checkbox行，选中行前，先清掉已选择行
			_638(_633);
			_637.splice(0, _637.length);
		}
		//        else{   //支持多选,如果不存在已选择的checkbox，并且设置为单击行不选中checkbox，则清掉数据后在选择，
		//                //如果存在已选择的checkbox，单击行则同点击checkbox点击
		//                if(!trs){
		//                   _638(_633);
		//			       _637.splice(0, _637.length); 
		//                }
		//        }
		if (!_635 && trs.length > 0) { // diy 关于单击行，不选中checkbox
			_639(_633, _634, true);
		}
	...
	_6ba(["text", "textbox", "numberbox", "numberspinner", "combobox", "combotree", "combogrid", "datebox", "datetimebox", "timespinner", "datetimespinner"]), {
		...
		validatebox: {
			init: function(_6d8, _6d9) {
				var _6da = $("<input type=\"text\" class=\"datagrid-editable-input\" onfocus=\"this.select()\" >").appendTo(_6d8);
	...
	var _711 = {
		...
		//
		// 重写datagrid的renderFooter方法
		// target：datagrid目标
		// _6db
		// frozen：是否锁定列
		//
		renderFooter: function(target, _6db, frozen) {
			var opts = $.data(target, "datagrid").options;
			var rows = $.data(target, "datagrid").footer || [];
			var fields = $(target).datagrid("getColumnFields", frozen);
			var footerhtml = ["<table class=\"datagrid-ftable\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\"><tbody>"];
			for (var i = 0; i < rows.length; i++) {
				footerhtml.push("<tr class=\"datagrid-footer-row\" datagrid-row-index=\"" + i + "\">");
				footerhtml.push(this.renderRow.call(this, target, fields, frozen, i, rows[i], true));
				footerhtml.push("</tr>");
			}
			footerhtml.push("</tbody></table>");
			//$(_6db).html(footerhtml.join(""));
            $(_6db)[0].innerHTML = footerhtml.join("");
		},

		//用于数字四舍五入
		MathRoundTo: function(number, digit) {
		...
		//
		// 重写datagrid的rendRow方法
		// target：datagrid目标
		// fields：所有列
		// frozen：是否锁定列
		// rowIndex：行号
		// rowData：行数据源
		// isfooter：是否是datagrid脚本行，这个是扩展参数，需要修改datagrid里renderFooter里面的方法
		//
		renderRow: function(target, fields, frozen, rowIndex, rowData, isfooter) {
			var opts = $.data(target, "datagrid").options;
			var rowhtml = [];
			if (frozen && opts.rownumbers) {
				...
				//Diy 自己修改
				//rowhtml.push("<td class=\"datagrid-td-rownumber\"><div class=\"datagrid-cell-rownumber\">" + rownumber + "</div></td>");
                //增加了一个合计样式
				if (isfooter) rowhtml.push("<td class=\"datagrid-td-rownumber\"><div class=\"datagrid-cell-rownumber\"><div class=\"datagrid-footer-rownumber\">合计</div></div></td>");
				else rowhtml.push("<td class=\"datagrid-td-rownumber\"><div class=\"datagrid-cell-rownumber\">" + rownumber + "</div></td>");
			}
			for (var i = 0; i < fields.length; i++) {
				...
				//                    sum: true
				//                    boxWidth: 48
				//                    cellClass: datagrid-cell-c1-total
				if (col) {
					var field_value = rowData[field]; //value = rowData[field]
					var css = col.styler ? (col.styler(field_value, rowData, rowIndex) || "") : "";
					var _6e7 = "";
					var _6e8 = "";
					if (typeof css == "string") {
						_6e8 = css;
					...
					var cell_style = col.hidden ? "style=\"display:none;" + _6e8 + "\"": (_6e8 ? "style=\"" + _6e8 + "\"": "");
					rowhtml.push("<td field=\"" + field + "\" " + cls + " " + cell_style + ">");
					var cell_style = "";
					if (!col.checkbox) {
						if (col.align) {
							cell_style += "text-align:" + col.align + ";";
						}
						if (!opts.nowrap) {
							cell_style += "white-space:normal;height:auto;";
						} else {
					...
					rowhtml.push("<div style=\"" + cell_style + "\" ");

					rowhtml.push(col.checkbox ? "class=\"datagrid-cell-check\"": "class=\"datagrid-cell " + col.cellClass + "\"");
					rowhtml.push(">");
					if (col.checkbox) {
						if (!isfooter) {
							rowhtml.push("<input type=\"checkbox\" " + (rowData.checked ? "checked=\"checked\"": ""));
							rowhtml.push(" name=\"" + field + "\" value=\"" + (field_value != undefined ? field_value: "") + "\">");
						}
					} else {
						if (col.formatter) {
							if (isfooter && col.columntype == 'CustomColumn') 
								rowhtml.push("");
							else 
								rowhtml.push(col.formatter(field_value, rowData, rowIndex));
						} else if (col.columntype == 'DateColumn') {
							//ColumnType:TextColumn,NumberColumn,PercentColumn,DateColumn,DateTimeColumn,RowDeleteColumn,ImageColumn;
							if (!field_value) rowhtml.push("");
							else {
								field_value = field_value.replace(/-/g, "/");
								var datetime = new Date(field_value);
								var y = datetime.getFullYear();
								if (isNaN(y)) rowhtml.push(field_value);
								else {
									var m = datetime.getMonth() + 1;
									var d = datetime.getDate();
									var rec = y + '-' + (m < 10 ? ('0' + m) : m) + '-' + (d < 10 ? ('0' + d) : d);
									rowhtml.push(rec);
								}
							}
						} else if (col.columntype == "DateTimeColumn") {
							if (!field_value) rowhtml.push("");
							else {
								field_value = field_value.replace(/-/g, "/");
								var datetime = new Date(field_value);
								...
								var rec = y + '-' + (m < 10 ? ('0' + m) : m) + '-' + (d < 10 ? ('0' + d) : d) + ' ' + (h < 10 ? ('0' + h) : h) + ':' + (mm < 10 ? ('0' + mm) : mm) + ':' + (s < 10 ? ('0' + s) : s);
								rowhtml.push(rec);
							}
						} else if (col.columntype == "PercentColumn") {
							if (!field_value) field_value = 0;
							rowhtml.push(field_value + "%");

						} else if (col.columntype == "NumberColumn") {
							if ((!field_value || field_value == "") && isfooter) 
                                rowhtml.push("");
							else {
								if (!field_value || field_value == "") field_value = 0;

								//如果decimaldigits与decimalscale同时存在，则以decimaldigits为准
								//decimaldigits四舍五入保留小叔位数，不够长补0。值必须为正整数，默认值为2；
								var decimaldigits = col.decimaldigits;
								//if(!decimaldigits) decimaldigits = 2;
								//decimalscale四舍五入保留小叔位数，不够长不补0。值必须为正整数，默认值为2；
								var decimalscale = col.decimalscale;
								//if(!decimalscale) decimalscale = 2;
								//如果decimaldigits与decimalscale同时存在或者都不存在，则以decimaldigits为准
								if (decimaldigits && decimalscale) decimalscale = undefined;
								if (!decimaldigits && !decimalscale) decimaldigits = 2;

								//displayemptyforzero 是否0时不显示。默认为false
								var displayemptyforzero = col.displayemptyforzero;
								if (!displayemptyforzero) displayemptyforzero = false;

								//NumberType：可选择的值有：Float，PositiveFloat,Int,positiveInt
								//如果没有设置numbertype类型，则默认为PositiveFloat。
								var numbertype = col.numbertype;
								if (!numbertype) numbertype = "PositiveFloat";
								var num = Number(field_value);
								if (numbertype == "positiveInt" || numbertype == "PositiveFloat") {
									if (num < 0) num = 0; //如果设置的是正整数或者正浮点数，如果value < 0,修正为0
								}

								if (num == 0 && displayemptyforzero) rowhtml.push("");
								else {
									if (decimaldigits) {
										var num = this.MathRoundTo.call(this, field_value, decimaldigits);
										var numstring = num.toString();
										var index = numstring.indexOf('.');
										var substr = "";
										if (index > 0) {
											var numstr = numstring.split('.');

											if (numstr.length == 2) substr = numstr[1];
											var sublen = substr.length;
											if (sublen != decimaldigits) {
												for (var j = 0; j < decimaldigits - sublen; j++) {
													substr += "0";
												}
											}
											field_value = numstr[0] + "." + substr;
											rowhtml.push(field_value);
										} else {

											for (var j = 0; j < decimaldigits; j++) {
												substr += "0";
											}
											field_value = num + "." + substr;
											rowhtml.push(field_value);
										}
									} else {
										var num = this.MathRoundTo.call(this, field_value, decimalscale);
										rowhtml.push(num);
									}
								}
							}
						} else if (col.columntype == "TextColumn") {
							var itemlist="";
							try {
                                itemlist="";
								itemlist = col.itemlist.toString();
							} catch(e) {
                                itemlist="";
                            }
							...
							if (isfooter) {
								...
								if (!itemlist || itemlist == "" || itemlist == undefined) {
                                    rowhtml.push(field_value);
                                }
								else {
									...
									for (var j = 0; j < items.length; j++) {
										...
										if (field_value == subitems[0]) {
											rowhtml.push(subitems[1]);
											break;
										}
									}
								}
	...
	$.fn.panel.defaults, {
		...
		loadFilter: function(data) {
			if (!data || data == undefined) { // diy 增加容错判断
				return {
					total: 0,
					rows: []
				};
